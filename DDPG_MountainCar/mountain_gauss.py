...
def trainer(epochs=1000, MINIBATCH_SIZE=40, GAMMA = 0.99, epsilon=1.0, min_epsilon=0.01, BUFFER_SIZE=10000, train_indicator=True, render=False):
    with tf.Session() as sess:
        ...
        std_dev = (env.action_space.high-env.action_space.low)/5
        ...
        for i in range(epochs):
            ...
            while (not done):
                ...
                action = action_original + np.random.normal(loc=0, scale=std_dev, size=action_dim)
...
if __name__ == '__main__':
    ...
    np.save('rewards_gauss_5', rewards)
