...
def trainer(epochs=1000, MINIBATCH_SIZE=40, GAMMA = 0.99, epsilon=1.0, min_epsilon=0.01, BUFFER_SIZE=10000, train_indicator=True, render=False):
    with tf.Session() as sess:
        ...
        sigma = np.array((env.high_state-env.low_state)/5.)
        ...
        for i in range(epochs):
            ...
            while (not done):
                if render:
                    env.render()
                state[0] += np.random.normal(loc=0, scale=sigma[0])
                state[1] += np.random.normal(loc=0, scale=sigma[1])
...
if __name__ == '__main__':
    ...
    np.save('rewards_param_space_5', rewards)